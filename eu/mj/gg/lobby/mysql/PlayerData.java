...
public class PlayerData {
	...
	protected boolean getPjump() {
		boolean pjump = cfg.getBoolean("pjump." + p.getName());
		return pjump;
	}

	protected void setPjump(boolean pjump) {
		cfg.set("pjump." + p.getName(), pjump);
		try {
			cfg.save(f);
		} catch (IOException e) {
			e.printStackTrace();
	...
	protected boolean getSilent() {
		boolean Silent = cfg.getBoolean("silent." + p.getName());
		return Silent;
	}

	protected void setSilent(boolean Silent) {
		cfg.set("silent." + p.getName(), Silent);
		try {
			cfg.save(f);
		} catch (IOException e) {
			e.printStackTrace();
	...
	public boolean getWjump() {
		boolean Wjump = cfg.getBoolean("wjump." + p.getName());
		return Wjump;
	}

	public void setWjump(boolean wjump) {
		cfg.set("wjump." + p.getName(), wjump);
		try {
			cfg.save(f);
		} catch (IOException e) {
			e.printStackTrace();
	...
	public boolean getRide() {
		boolean ride = cfg.getBoolean("ride." + p.getName());
		return ride;
	}

	public void setRide(boolean ride) {
		cfg.set("ride." + p.getName(), ride);
		try {
			cfg.save(f);
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void saveInAsyncMySQL() {
		for (OfflinePlayer p : Bukkit.getOfflinePlayers()) {
			String uuid = p.getUniqueId().toString();
			LobbyAPI.setColor(uuid, (Integer) new PlayerData(p.getName()).getColor());
			try {
				LobbyAPI.setPjump(uuid, new PlayerData(p.getName()).getPjump());
			} catch (SQLException e) {
				e.printStackTrace();
			}
			try {
				LobbyAPI.setWjump(uuid, new PlayerData(p.getName()).getWjump());
			} catch (SQLException e) {
				e.printStackTrace();
			}
			try {
				LobbyAPI.setRide(uuid, new PlayerData(p.getName()).getRide());
			} catch (SQLException e) {
				e.printStackTrace();
			}
		}
	...
	protected void getPlayerData() {
		if (!cfg.contains(p.getName())) {
			...
			cfg.set("ride." + p.getName(), true);
			cfg.set(p.getName(), 0);
			cfg.set("UUID." + p.getName(), p.getUniqueId());
			try {
				cfg.save(f);
			} catch (IOException e) {
				e.printStackTrace();
	...
	public void getAsyncMySQL() {
		for (OfflinePlayer p : Bukkit.getOfflinePlayers()) {
			String uuid = p.getUniqueId().toString();
			boolean wjump = LobbyAPI.getWjump(uuid);
			boolean pjump = LobbyAPI.getPjump(uuid);
			int color = LobbyAPI.getColor(uuid);
			boolean ride = LobbyAPI.getRide(uuid);
			boolean silent = LobbyAPI.getSilent(uuid);
			cfg.set("wjump." + p.getName(), wjump);
			cfg.set("pjump." + p.getName(), pjump);
			cfg.set("silent." + p.getName(), silent);
			cfg.set("ride." + p.getName(), ride);
